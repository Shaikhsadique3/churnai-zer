...
serve(async (req) => {
  console.log("Function process-churn-data/index.ts - START"); // Debug log
  ...
  try {
    ...
    console.log(`Input Summary: Upload ID: ${upload_id}`); // Debug log

    if (!upload_id) {
      console.error("Error: Upload ID is required."); // Log missing upload ID
      console.log("Function process-churn-data/index.ts - END (Error: Missing Upload ID)"); // Debug log
      return new Response(
    ...
    if (fetchError) {
      ...
      console.log("Function process-churn-data/index.ts - END (Error: Upload not found)"); // Debug log
    ...
    console.log(`Output Summary: Analysis Results: High Risk: ${analysisResults.high_risk_percentage}%, Medium Risk: ${analysisResults.medium_risk_percentage}%, Low Risk: ${analysisResults.low_risk_percentage}%`); // Debug log
    ...
    if (reportUploadError) {
      ...
      console.log("Function process-churn-data/index.ts - END (Error: Failed to upload report)"); // Debug log
    ...
    if (insertReportError) {
      ...
      console.log("Function process-churn-data/index.ts - END (Error: Failed to save report record)"); // Debug log
    ...
    if (emailError) {
      ...
      console.log("Function process-churn-data/index.ts - END (Error: Failed to send email)"); // Debug log
    } else {
      console.log("Email sent successfully:", emailData); // Log email send success
      console.log(`Output Summary: Email Status: Success, Report URL: ${publicUrl}`); // Debug log
    }

    console.log("Process Churn Data function finished successfully."); // Log function end
    console.log("Function process-churn-data/index.ts - END (Success)"); // Debug log
    ...
    console.log("Function process-churn-data/index.ts - END (Internal Server Error)"); // Debug log
...
function generateLiteReport(results: any): string {
  console.log("Function generateLiteReport - START"); // Debug log
  console.log(`Input Summary: Results: ${JSON.stringify(results)}`); // Debug log
  const report = `
...
${new Date().toISOString()}
  `;
  console.log("Function generateLiteReport - END (Success)"); // Debug log
  return report;
}